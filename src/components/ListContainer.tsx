...
const ListContainer = ({ boardId, list }: ListProps) => {
  ...
  const onDragNDrop = async (result: any) => {
    ...
    if (type == 'list') {
      const data = reOrderData(listData, destination.index, source.index).map((item: any, index: number) => ({ ...item, order: index }))
      setListData(data)
      ...
      <Droppable droppableId='lists' type='list' direction='horizontal'>
        {(provided) => (
          <ol {...provided.droppableProps} ref={provided.innerRef} className='flex h-full gap-x-3'>
            {listData?.map((list: List, index: number) => <ListItem key={list.id} list={list} index={index} />)}
